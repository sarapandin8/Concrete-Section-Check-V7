...
def _render_beam_uls_static_plotly_figure(fig: go.Figure, *, caption: str | None = None) -> None:
    """Render Beam/Girder ULS diagrams without backend image export.

    STATE.RESULT.PERSIST3A: a deployed Streamlit session can appear to run
    forever after pressing Calculate when backend Plotly/Kaleido image export is
    invoked on the server.  The engineering result is already stored before the
    diagram is rendered, so the Analysis page must not depend on backend PNG
    export.  Render the Plotly figure directly in the browser and keep the
    calculation path free of Kaleido/image-export side effects.
    """
